...
L&#39;API CMU fornisce una visualizzazione gerarchica dei cubi OLAP sottostanti. Ogni risorsa ([dimensione](/help/concurrency-monitoring/reports/cm-usage-reports.md#dimensions-2-filter-metrics) nella gerarchia delle dimensioni, mappata come segmento di percorso URL) genera rapporti con [metriche](/help/concurrency-monitoring/reports/cm-usage-reports.md#monitor-metrics) (aggregate) per la selezione corrente. Ogni risorsa punta alla relativa risorsa padre (per l’aggregazione) e alle relative risorse secondarie (per l’espansione). Le operazioni di suddivisione in porzioni e dicing vengono eseguite tramite parametri di stringa di query che fissano dimensioni a valori o intervalli specifici.
...
* [Panoramica CMU](/help/concurrency-monitoring/reports/cm-usage-reports.md)
