...
Una volta completato il flusso di logout, l’iFrame verrà reindirizzato a un endpoint di autenticazione Adobe Pass personalizzato. Questo distribuirà uno snippet JS che esegue un `window.postMessage` all&#39;elemento padre, notificando all&#39;AccessEnabler il completamento della disconnessione. Vengono attivati i seguenti callback: `setAuthenticationStatus()` e `sendTrackingData(AUTHENTICATION_DETECTION ...)`, che segnala che l’utente non è più autenticato.
