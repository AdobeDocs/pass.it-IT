...
iOS AccessEnabler è una libreria Objective C iOS/tvOS che consente alle app mobili di utilizzare l’autenticazione Adobe Pass per i servizi di adesione di TV Everywhere. L&#39;implementazione consiste nel *AccessEnabler* che definisce l&#39;API di adesione e il *EntitlementDelegate* e *[EntitlementStatus](#ios%20entitlement%20status)* protocolli che descrivono i callback attivati dalla libreria. L’interfaccia e il protocollo sono denominati con un nome comune: libreria AccessEnabler.

## Requisiti di iOS e tvOS {#reqs}

Per i requisiti tecnici correnti relativi alla piattaforma iOS e tvOS e all’autenticazione di Adobe Pass, consulta [Requisiti di piattaforma/dispositivo/strumento](#ios)e consultare le note sulla versione incluse nel download dell’SDK. Nel resto di questa pagina, vedrai sezioni che annotano le modifiche che si applicano a particolari versioni SDK e superiori. Ad esempio, la seguente è una nota legittima relativa all’SDK 1.7.5:
