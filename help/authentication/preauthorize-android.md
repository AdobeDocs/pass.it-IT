...
In caso di errore imprevisto (ad esempio, problema di rete, endpoint di autorizzazione MVPD non disponibile, ecc.) in corso, quando una richiesta API di preautorizzazione viene elaborata dai servizi di autenticazione di Adobe Pass, verranno incluse una o più informazioni di errore separate per le risorse interessate come parte del risultato della risposta API di preautorizzazione.
